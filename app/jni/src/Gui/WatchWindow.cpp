...
void WatchWindow::ShowRX() {
    ...
    for (int i = 0; i < 16; i++) {
        ImGui::SameLine();
        sprintf(id, "##data%d", i);
        ImGui::SetNextItemWidth(char_width * 3 * UI::Scaling::fontScale);
        ImGui::Text("%s", (char*)&reg_rx[i][0]);
    }
    ImGui::Text("ERn: ");
    for (int i = 0; i < 16; i += 2) {
        ImGui::SameLine();
        uint16_t val = m_emu->chipset.cpu.reg_r[i + 1] << 8 | m_emu->chipset.cpu.reg_r[i];
        ImGui::Text("%04x ", val);
    }
    ...
    auto show_sfr = ([&](char* ptr, const char* label, int i, int width = 4) {
        ...
        ImGui::SameLine();
        sprintf(id, "##sfr%d", i);
        ImGui::SetNextItemWidth((char_width * width + 5) * UI::Scaling::fontScale);
        ImGui::Text("%s", ptr);
    });
...
void WatchWindow::RenderCore() {
    // Cập nhật các giá trị scale
    UI::Scaling::UpdateUIScale();

    char_width = ImGui::CalcTextSize("F").x * UI::Scaling::fontScale;
    casioemu::Chipset& chipset = m_emu->chipset;
    ImGui::BeginChild("##reg_trace", ImVec2(0, ImGui::GetTextLineHeightWithSpacing() * 4 * UI::Scaling::fontScale), false, 0);
    PrepareRX();
    ShowRX();
    ...
    ImGui::Separator();
    static int range = 64;
    ImGui::BeginChild("##stack_trace", ImVec2(0, ImGui::GetWindowHeight() / 2 * UI::Scaling::fontScale));
    std::string s = chipset.cpu.GetBacktrace();
    ImGui::InputTextMultiline("##as", (char*)s.c_str(), s.size(), ImVec2(ImGui::GetWindowWidth() * UI::Scaling::fontScale, -1), ImGuiInputTextFlags_ReadOnly);
    ImGui::EndChild();
