...
void MemBreakPoint::DrawContent() {
    ...
    while (c.Step()) {
        for (int i = c.DisplayStart; i < c.DisplayEnd; i++) {
            ...
            // Track press duration for each item
            float current_time = ImGui::GetTime();

            if (ImGui::Selectable(buf, selected == i)) {
                // Handle normal click
                if (current_time - pressStartTime < LONG_PRESS_DURATION) {
                    selected = i;
                }
            }
            ...
            if (ImGui::IsItemActive()) {
                if (pressedItem != i) {
                    pressedItem = i;
                    pressStartTime = current_time;
                }
                else if (current_time - pressStartTime >= LONG_PRESS_DURATION) {
                    ImGui::OpenPopup(("context_menu_" + std::to_string(i)).c_str());
                    pressedItem = -1;
                }
            }

            if (ImGui::BeginPopup(("context_menu_" + std::to_string(i)).c_str())) {
                selected = i;
                ImGui::Text(
#if LANGUAGE == 2
                    "请选择断点模式:"
...
#else
            "No breakpoints. Please add a breakpoint and long press to set mode."
#endif
        );
        return;
    }
