...
void CodeViewer::RenderCore() {
    // Update scaling from shared namespace
    UI::Scaling::UpdateUIScale();

    int h = ImGui::GetTextLineHeight() + UI::Scaling::padding;
    ...
    sz.x = w;
    sz.y = h;
    ImGui::BeginChild("##scrolling", ImVec2(0, -UI::Scaling::buttonHeight - UI::Scaling::padding));
    DrawContent();
    ImGui::EndChild();
    ImGui::SameLine();
    ImGui::Separator();
